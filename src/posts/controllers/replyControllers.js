const {
  ...
  updateReply,
  deleteReply,
} = require("../services/replyServices");
...
module.exports.getReplies = async (req, res) => {
  ...
  try {
    ...
    replies
      ...
      : res
          .status(404)
          .json({ success: false, message: "Post has no replies" });
  } catch (err) {
  ...
  try {
    ...
    if (reply.author._id != id) {
      return res
        ...
        .json({
          success: false,
          message: "Post can only be editted by author",
        });
    }
  ...
  try {
    ...
    if (reply.author._id != id) {
      return res
        ...
        .json({
          success: false,
          message: "Reply can only be deleted by author",
        });
    }
